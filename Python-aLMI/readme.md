...
Here again, the user should double-check that the path in the above command is indeed the correct path to use on their respective system.
...
1. a configuration (`.ini`) file of input parameters for the aLMI run, which, among others, contains the path to the SIOP sets file (`.nc` file) to use; and 
...
* Section of USER INPUTS:
	...
	- `CONFIG_FILE`: configuration file for this job (`.ini` file)
	- `INPUT_FILE`: file of input data (`.nc` or `.hdf` file)
...
One of the main inputs to `aLMI_main.py` is the configuration `.ini` file of aLMI input parameters. This section provides a brief description of the various parameters that can be used and set in this type of file. Additionally, the example `.ini` files in the `ini_files` directory can be investigated to find some typical values for these parameters.
...
* Section `[inputParameters]`:
	...
	- `inputSpectrumVarNames`: list of strings representing the names of the reflectance bands that will be used by aLMI. **Note**: these must match the band names in the input data file to aLMI.  
	_Example:_ inputSpectrumVarNames = \["Rrs_412" , "Rrs_443" , "Rrs_488"\]
	- `useWavelengths`: vector of wavelengths (in nm) for which to extract the SIOPs from the SIOP sets file. **Note**: this vector must have the same number of elements as `inputSpectrumVarNames`.  
...
* Section `[outputParameters]`:
	- `outWavelengthLabels`: vector of labels (strings) to be used in the labelling of the output variables. **Note**: this vector must have the same number of elements as `inputSpectrumVarNames`, and the values must be within `tolerance` of `useWavelengths`.  
...
* Section `[optionalParameters]`:
	...
	- `SIOP_SETS_10nm_FILE`: path to a file of SIOP sets (`.nc` file) with 10nm-spaced wavelengths to use if any of `Kd_par_MIM`, `Kd_490_MIM`, or `SD_MIM` is specified (see below). **Note:** calculation of Kd and Secchi depth estimates (i.e. using any of these three options) also requires the input file of reflectances to contain the solar zenith angle in a variable labelled `zen`.  
...
One aspect of Gadi is that the cost (to the selected NCI project, in Service Units) of a batch job execution depends the largest contribution from either the amount of CPU time required by the job (i.e. number of CPUs multiplied by walltime), or the requested amount of memory (proportionally to the total amount of memory available on each node). The previous NCI system (Raijin) only accounted for the CPU time in the cost calculations.
